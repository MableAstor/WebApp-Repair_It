...
export default function ProcessDetails() {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/repair-requests/${id}`);
        ...
        console.log("PROCESS DETAIL =", result);
  ...
  const locationText = data?.location
      ? [
        ...
        `อาคาร ${String(data.location.building).replace(/building/i, "")}`,
  ...
  const urgencyText = data?.severity || "ไม่ระบุ";

  const imageList =
      data?.images?.map((img) => {
        if (!img?.fileName) return "";
        return `${API_BASE}/uploads/${img.fileName}`;
      }).filter(Boolean) || [];
  ...
  return (
      <div className="min-h-screen bg-[#CDBCDB] font-sans flex flex-col">
        <Navbar title="Detail" showBack={true} />
